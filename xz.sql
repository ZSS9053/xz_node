...
--
-- Host: 127.0.0.1
-- Generation Time: 2017-11-15 02:48:40
...
-- 存储过程
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp1` ()  BEGIN
	SELECT 1+2;
   SELECT * FROM xz_user WHERE uid>2;
   SELECT * FROM xz_receiver_address;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp2` (IN `num1` INT, IN `num2` INT)  BEGIN
	DECLARE sum INT DEFAULT 0;
   SET sum=sum+num1;
   SELECT sum+num2 INTO sum;
   SELECT sum;
   SELECT * FROM xz_user WHERE uid>=num1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp3` (IN `userId` INT, OUT `uc` INT)  BEGIN
  SELECT COUNT(*) INTO uc FROM xz_user WHERE uid>=userId;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `userLogin` (IN `userName` VARCHAR(32), IN `userPwd` VARCHAR(32), OUT `result` VARCHAR(8))  BEGIN
	DECLARE userId INT DEFAULT 0;
   SELECT uid INTO userId FROM xz_user WHERE uname=userName AND upwd=userPwd;
   IF userId>0 THEN SET result='succ';
   ELSE SET result='err';
   END IF;
END$$
...
(1, 'Apple MacBook Air系列', '酷睿双核i5处理器|256GB SSD|8GB内存|英特尔HD显卡620含共享显卡内存', 'img/index/study_computer_img1.png', '6988.00', 'product_details.html?lid=1', 1, 6, 0),
(2, '小米Air 金属超轻薄', '酷睿双核i5处理器|512GB SSD|2GB内存|英特尔HD独立显卡', 'img/index/study_computer_img2.png', '3488.00', 'product_details.html?lid=5', 2, 0, 5),
(3, '联想E480C 轻薄系列', '酷睿双核i7处理器|256GB SSD|4GB内存|英特尔HD显卡680M', 'img/index/study_computer_img3.png', '5399.00', 'product_details.html?lid=9', 3, 5, 4),
(4, '华硕RX310 金属超极本', '酷睿双核i5处理器|512GB SSD|4GB内存|英特尔HD游戏级显卡', 'img/index/study_computer_img4.png', '4966.00', 'product_details.html?lid=13', 4, 4, 3),
(5, '联想小新700 电竞版游戏本', '酷睿双核i7处理器|1TGB SSD|8GB内存|英特尔HD显卡620含共享显卡内存', 'img/index/study_computer_img5.png', '6299.00', 'product_details.html?lid=17', 5, 2, 1),
(6, '戴尔灵越燃7000 轻薄窄边', '酷睿双核i5处理器|512GB SSD|2GB内存|英特尔HD显卡', 'img/index/study_computer_img3.png', '5199.00', 'product_details.html?lid=19', 6, 3, 2),
(7, '神州战神Z7M 高性价比游戏本', '酷睿双核i7处理器|1TGB SSD|8GB内存|英特尔HD游戏机独立显卡', 'img/index/study_computer_img4.png', '5799.00', 'product_details.html?lid=38', 0, 1, 6);
...
(5, 'huahua', '1234', 'huahua@163.com', '', NULL, '', 0),
(6, 'qin', 'abcd', 'qin@qq.com', NULL, NULL, NULL, NULL),
(11, 'zss', '1234', NULL, NULL, NULL, NULL, NULL),
(12, 'zss1', '1234', NULL, NULL, NULL, NULL, NULL),
(13, 'zss1', '1234', NULL, NULL, NULL, NULL, NULL),
(15, 'test10', 'testupwd10', '10@tedu.cn', '135010101010', NULL, NULL, NULL),
(16, 'huahua1', '1234', '1234', '1234', NULL, NULL, NULL),
(17, 'huahua1', '1234', '1234', '1234', NULL, NULL, NULL),
(18, 'huahua1', '123456', '123456@qq.com', '12345678901', NULL, NULL, NULL),
(19, 'huahua1', '123456', '123456@qq.com', '12345678901', NULL, NULL, NULL),
(20, 'huahua12', '123456', '123@qq.com', '12345678012', NULL, NULL, NULL),
(21, 'huahua4', '123456', '321@qq.com', '12365478902', NULL, NULL, NULL);
